...
void Renderer::init(int width, int height, const std::string& objectPath, const std::string& texturePath){
	...
	// Initialize objects.
	_weyr.init(objectPath, texturePath);
	_skybox.init();
...
void Renderer::draw(){
	...
	// Draw objects
	_weyr.draw(elapsed, _camera._view, _camera._projection, _pingpong);
...
void Renderer::keyPressed(int key, int action){
	if(action == GLFW_PRESS){
		if(key == GLFW_KEY_O){
			_weyr._primitiveScale -= 0.005f;
			_weyr._primitiveScale = std::max(0.0f,_weyr._primitiveScale);
