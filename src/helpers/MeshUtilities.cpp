...
void loadObj(const std::string & filename, mesh_t & mesh, LoadMode mode){
	...
	if (mode == Points){
		// Mode: Points
		// In this mode, we don't care about faces. We simply associate each vertex/normal/uv in the same order. If faces are defined, we recompute normals.
		...
		if(hasNormals){
			if(faces_temp.empty()){
				...
				for(size_t i = 0; i < faces_temp.size(); ++i){
					...
					mesh.normals[ind1] += normals_temp[ind3];
				}
				for(size_t j = 0; j < mesh.normals.size(); ++j){
					mesh.normals[j] = normalize(mesh.normals[j]);
				}
			}
		}
